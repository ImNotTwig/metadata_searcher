...
//////////////////Getting artists from the user to search the metadata for/////////////////////

    let mut artist = "a".to_string();
    let mut artists_list = vec![];
    let mut counter = 0;
    while artist.chars().take(1).last().unwrap() != '\n'  {
        if counter == 0 {
            println!("which artists would you like to query?");
            println!("When done typing full name of artist press enter.");
            println!("If you dont want to query anymore artists press enter without putting an artist");
            counter = 1;
        }else {
            println!("");
        }
        ...
        if artist.chars().take(1).last().unwrap() == '\n' {
            break
        }
        artists_list.push(artist.clone());
    }
